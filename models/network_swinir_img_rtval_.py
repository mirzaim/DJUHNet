...
class SwinIR(nn.Module):
    ...
    def forward_features(self, x, img_rep):
    ...
    def forward(self, x):
        ...
        if self.rep_vec_list:
            # img_rep = torch.cat((*self.rep_vec_list, ), dim=1)
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, img_rep)) + x
            ...
            res = self.conv_after_body(self.forward_features(x_first, img_rep)) + x_first
