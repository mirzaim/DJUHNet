...
def main():
    ...
    mAP5000 = CalcTopMap(fvs_index, fvs_query_L,
                          label_index, label_query, 5000)
    mAP = CalcTopMap(fvs_index, fvs_query_L, label_index, label_query, -1)
    print('LQ')
    print(f'mAP5000 result {mAP5000}')
    print(f'mAP result {mAP}')

    mAP5000 = CalcTopMap(fvs_index, fvs_query_SR,
                          label_index, label_query, 5000)
    ...
    # mAP, cum_prec, cum_recall = CalcTopMapWithPR(fvs_index, fvs_query_SR,
    #                                              label_index, label_query, 5000)
    # curve_data = get_curve_data(mAP, cum_prec, cum_recall, label_index.shape[0])
    # with open(str(args.hash_bit) + 'bit_5000.json', 'w') as f:
    #     f.write(json.dumps(curve_data))

    # mAP, cum_prec, cum_recall = CalcTopMapWithPR(fvs_index, fvs_query_SR,
    #                                              label_index, label_query, -1)
    # curve_data = get_curve_data(mAP, cum_prec, cum_recall, label_index.shape[0])
    # with open(str(args.hash_bit) + 'bit.json', 'w') as f:
    #     f.write(json.dumps(curve_data))

    mAP5000 = CalcTopMap(fvs_index, fvs_query_H,
                          label_index, label_query, 5000)
    mAP = CalcTopMap(fvs_index, fvs_query_H, label_index, label_query, -1)
    print('HQ')
    print(f'mAP5000 result {mAP5000}')
    print(f'mAP result {mAP}')
